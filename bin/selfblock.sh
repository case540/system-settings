...
BLOCKLIST=("m.wikipedia.org" "www.wikipedia.org" "wikipedia.org" "www.amazon.com" "m.amazon.com" "www.facebook.com" "m.facebook.com", "netflix.com", "www.netflix.com", "hulu.com", "www.hulu.com")
BLOCK_BASE="0.0.0.0"
ETC_FILE="/etc/hosts"
DELAY_IN_SEC=300
...
function resume_block()
{
    ...
    do
        ...
        if [ "$PUNISH" != "no" ]; then
            ps aux | grep [t]erminator | awk '{print $2}' | xargs -I {} kill -9 {};
            sed -i.bak "/\/etc\/hosts/d" ~/.bash_history
            sed -i.bak "/selfblock.sh/d" ~/.bash_history
            PUNISH="no";
        fi
    done
}
